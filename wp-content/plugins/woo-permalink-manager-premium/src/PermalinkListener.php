...
class PermalinkListener {
	...
	public function __construct() {
		...
		if ( premmerce_wpm_fs()->is__premium_only() ) {
			#/premmerce_clear
			$this->taxonomyOptions['product_tag'] = isset( $options['tag'] ) ? $options['tag'] : '';
			#premmerce_clear
		}
		#/premmerce_clear
	}
	...
	public function registerFilters() {
		...
		add_filter( 'term_link', [ $this, 'replaceTermLink' ], 0, 3 );
	...
	public function replaceProductLink( $permalink, $post ) {
        ...
        $permalink = $this->replaceSlugWithSku( $permalink, $post->ID );
      }
    }
		...
		if ( premmerce_wpm_fs()->is__premium_only() ) {
			if ( $this->options['enable_suffix_products'] ) {
				...
				if ( $suffix ) {
					...
					if( isset( $linkQuery ) ) {
					  $linkQuery = '?' . $linkQuery;
            $replaced = str_replace( $linkQuery, '', $link );
					  $link     = $replaced . $suffix . $linkQuery;
          } else {
					  $link .= $suffix;
          }
				}
    ...
    if ( premmerce_wpm_fs()->is__premium_only() ) {
      if( $this->checkWpmlPlugin() ) {
        $link = $this->addParamsToPermalink( $link );
      }
    }
	...
	public function addRewriteRules( $rules ) {
		...
		foreach ( $this->taxonomyOptions as $taxonomy => $option ) {
			if ( ! empty( $option ) ) {
				...
				if ( premmerce_wpm_fs()->is__premium_only() ) {
					if ( $this->options['enable_suffix_categories'] ) {
	...
	private function getPolylangLangSlug() {
		if ( ! empty( $this->polyLang ) ) {
			...
			$languages = $this->polyLang->model->get_languages_list( array( 'fields' => 'slug' ) );
  ...
  protected function addParamsToPermalink( $permalink ) {
    ...
    if( $sitepress->get_default_language() != ICL_LANGUAGE_CODE && $isGetParamUrlFormat ) {
      return add_query_arg( array( 'lang' => ICL_LANGUAGE_CODE ), $permalink );
    }
